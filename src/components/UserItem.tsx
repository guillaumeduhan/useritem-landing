...
export default function UserItem(props: UserItemProps) {
  ...
  return <div className="useritem" style={style}>
    <div onClick={onClickItem} className={`useritem--item ${disabled ? 'useritem--disabled' : ''} ${loading ? 'useritem--loading' : ''} ${border ? 'border' : ''} ${border && shadow ? 'shadow' : ''} ${squared ? '' : 'rounded-[8px]'} ${reverse ? 'flex-row-reverse' : ''}`}>
